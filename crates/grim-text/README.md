...
- Layout helpers: `column_grid` (keyword grids) and `table(headers, rows, align)` (`src/table.rs`: header + dash separator + left/right-aligned rows for heterogeneous data like `sockets`; byte-widths, so pass already-escaped display strings).
