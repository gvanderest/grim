...
//! Names are registered into the shared [`CommandRegistry`] at startup (after
//! the static commands, so statics keep their prefixes); the handler renders
//! per-recipient [`InfoMessage`]s, the same primitive `tell` uses.
