...
- Look exit lines: `emit_look_room` (`src/look_output.rs`, `partition_exits`) splits the room's exits into `Exits: …` (plain exits plus open doors), `Doors: …` (closed doors), and `Secret: …` (hidden exits, admin-only) — all three on one line, each `none` when empty. Hidden exits list under `Secret:` regardless of open state (discovery is by walking through, not by listing). All lists sort in `Cardinal` display order (north, east, south, west, up, down); directions carry compass colours (north `{R`, south `{r`, east `{M`, west `{m`, up `{Y`, down `{y`, each self-terminated with `{x`). Room titles render `{B…{x` (blue, self-terminated).
