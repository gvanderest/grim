...
| Binary | File | Purpose |
|---|---|---|
| `example-mud` (default-run) | `src/main.rs` | The MUD server: `MinimalPlugins` (frame loop capped at 60 Hz via `ScheduleRunnerPlugin::run_loop` — bare `MinimalPlugins` spins idle at 99% CPU, issue #121) + `LogPlugin` + `GrimDefaultPlugins { telnet_port: 4000, ..Default::default() }`, seeded with `seed::seed_world`. |
