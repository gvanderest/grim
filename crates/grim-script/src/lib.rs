...
//! three globals — `rand`, `say`, and `event` — over a pure-data stdlib
//! (`math`, `string`, `table`, `utf8`); there is no `os`/`io`/`require`/`load`,
//! an instruction budget, and a memory cap, so a bad script can neither escape
//! nor hang the tick.
//!
